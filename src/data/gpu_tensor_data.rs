...
impl<'a> GpuTensorData<'a> {
    ...
    pub fn arbitrary_with_shape(shape: Shape) -> impl Strategy<Value = Self> {
        Self::arbitrary_with_shape_and_strategy(shape, -1.0f32..1.)
    }

    fn arbitrary_with_shape_and_strategy<S: Strategy<Value = f32>>(
        shape: Shape,
        strategy: S,
    ) -> impl Strategy<Value = Self> {
        let size = shape.size;
        let strides: Strides = (&shape).into();
        collection::vec(strategy, size).prop_map(move |data| {
