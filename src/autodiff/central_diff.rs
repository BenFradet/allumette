...
mod tests {
    ...
    fn central_diff_b_tests() -> () {
        assert!(is_close(1., central_diff_b(|a, b| a + b, 5., 10., true, 1e-6)));
        assert!(is_close(10., central_diff_b(|a, b| a * b, 5., 10., true, 1e-6)));
        assert!(is_close(5., central_diff_b(|a, b| a * b, 5., 10., false, 1e-6)));
    }
}