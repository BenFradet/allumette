    ...
    proptest! {
        ...
        fn reduce_test_sum(t1 in CpuTensorData::arbitrary()) {
            ...
            for i in 0..t1.shape.data().len() {
                t1p = TensorBackend::<f64, Seq>::reduce(&t1p, |a, b| a + b, i, 0.).unwrap();
            }
        ...
        fn reduce_test_mul(t1 in CpuTensorData::arbitrary()) {
            let mut t1p = t1.clone();
            for i in 0..t1.shape.data().len() {
                t1p = TensorBackend::<f64, Seq>::reduce(&t1p, |a, b| a * b, i, 1.).unwrap();
            }
        ...
        fn zip_commutative_test(t1 in CpuTensorData::arbitrary(), t2 in CpuTensorData::arbitrary()) {
            ...
            let res1 = TensorBackend::<f64, Seq>::zip(&t1, &t2, |a, b| a + b);
            let res2 = TensorBackend::<f64, Seq>::zip(&t2, &t1, |a, b| a + b);
            match (res1, res2) {
